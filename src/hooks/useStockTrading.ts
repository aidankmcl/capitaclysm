...
import { actions, useAppSelector, useAppDispatch } from "~/store";
import { useStockData, useStockHistory } from "~/hooks";
import {
  validateShares,
  canAffordPurchase,
  hasSufficientShares,
...
export const useStockTrading = (symbol: string) => {
  ...
  const handleTrade = () => {
    ...
    const tradeAction = action === "buy" ? actions.stocks.buyStock : actions.stocks.sellStock;
    dispatch(
      tradeAction({
